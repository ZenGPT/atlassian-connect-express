...
function authenticateInstall(addon) {
  return function (req, res, next) {
    ...
    // Install / Uninstall hook should always be asymmetric.
        ...
        res.format({
          ...
          html() {
            if (addon.config.errorTemplate()) {
              res.statusCode = code;
              res.render(addon.config.getErrorTemplateName(), (addon.config.getErrorTemplateObject()) ? { ...addon.config.getErrorTemplateObject(), message }: { message });
            } else {
              res.status(code).send(_.escape(message));
            }
          },
